...
export function parseText(text: string): ExtractedData {
    ...
    try {
        ...
        data.commentaires = extractors.extractAll(patterns.commentaire, text)
            .map(match => match[1]);
        ...
        // Ventes
        data.ventes = extractors.extractAll(patterns.venteAvecDate, text)
            .map(match => ({
                description: match[1],
        ...
        data.ventesStats = extractors.extractAll(patterns.venteStat, text)
            .map(match => ({
                description: match[1],
        ...
        data.boosts = extractors.extractAll(patterns.boost, text)
            .map(match => ({
                date: match[1],
                montant: parseFloat(match[2].replace(',', '.'))
            }));
        
        data.vitrines = extractors.extractAll(patterns.vitrine, text)
            .map(match => ({
                date: match[1],
                montant: parseFloat(match[2].replace(',', '.'))
            }));
        
        data.transferts = extractors.extractAll(patterns.transfert, text)
            .map(match => ({
                date: match[1],
                montant: parseFloat(match[2].replace(',', '.'))
            }));
        ...
        data.soldeFinal = soldeStr ? parseFloat(soldeStr.replace(',', '.')) : undefined;
