...
export class VintedAnalyzer {
  ...
  analyze() {
    try {
      ...
      console.log(
        "📝 Longueur du texte:",
        this.text.length.toLocaleString(),
        "caractères"
      );
      console.log("");
      ...
      const chiffreAffaires = this.result.ventes.reduce(
        (sum, v) => sum + v.prix,
        0
      );
      const prixMoyen = chiffreAffaires / totalVentes;
      const totalDepenses = this.result.depenses.reduce(
        (sum, d) => sum + d.montant,
        0
      );
      ...
      console.log(
        "⭐ Note:",
        "★".repeat(Math.round(this.result.boutique.note)),
        `(${this.result.boutique.note}/5)`
      );
      console.log(
        "👥 Abonnés:",
        this.result.boutique.abonnes?.toLocaleString() || 0
      );
      console.log(
        "👕 Articles en vente:",
        this.result.boutique.articles?.toLocaleString() || 0
      );
      console.groupEnd();
      ...
      if (this.result.ventes.length > 0) {
        ...
        this.result.ventes.slice(-5).forEach((vente, index) => {
          console.log(
            `   ${index + 1}. ${vente.nom} - ${vente.prix.toFixed(2)}€ (${
              vente.date
            })`
          );
        });
      ...
      if (this.result.marketing.boosts.length > 0) {
        ...
        this.result.marketing.boosts.slice(-3).forEach((boost, index) => {
          console.log(
            `     ${index + 1}. ${boost.article} - ${boost.montant.toFixed(
              2
            )}€ (${boost.date})`
          );
        });
      ...
      console.log(
        "   • Articles en vitrine:",
        this.result.marketing.dressingVitrine.length
      );
      if (this.result.marketing.dressingVitrine.length > 0) {
        console.log("   • Articles actuels:");
        this.result.marketing.dressingVitrine
          .slice(-3)
          .forEach((article, index) => {
            console.log(
              `     ${index + 1}. ${article.nom} - ${article.prix.toFixed(
                2
              )}€ (${article.date})`
            );
          });
      ...
      console.log(
        "💳 Solde actuel:",
        this.result.finances.soldeActuel.toFixed(2) + "€"
      );
      ...
      if (this.result.finances.transferts.length > 0) {
        ...
        this.result.finances.transferts
          .slice(-3)
          .forEach((transfert, index) => {
            console.log(
              `     ${index + 1}. ${transfert.montant.toFixed(2)}€ (${
                transfert.date
              })`
            );
          });
      ...
      console.log(
        "🎯 Taux de marge:",
        ((beneficeNet / chiffreAffaires) * 100).toFixed(1) + "%"
      );
      console.groupEnd();
  ...
  private _extractBoutiqueInfo() {
    ...
    this.result.boutique.stats.abonnes = parseInt(
      extractors.extract(patterns.abonnes, this.text) || "0"
    );
    this.result.boutique.stats.articlesActifs = parseInt(
      extractors.extract(patterns.articlesActifs, this.text) || "0"
    );
  ...
  private _analyserLanguesCommentaires(commentaires: string[]) {
    ...
    commentaires.forEach((commentaire, index) => {
      ...
      if (Object.keys(matchCount).length > 0) {
        const [langueDetectee] = Object.entries(matchCount).sort(
          ([, a], [, b]) => b - a
        )[0];
  ...
  private _extractVentes() {
    ...
    const ventesMatches = extractors.extractAll(
      patterns.venteAvecDate,
      this.text
    );
    ...
    const ventesStatMatches = extractors.extractAll(
      patterns.venteStat,
      this.text
    );
    ...
    if (ventesStatMatches.length > 0) {
      ...
      console.log(
        'Nombre de "vues" trouvées avec pattern simple:',
        vuesMatches ? vuesMatches.length : 0
      );
      if (vuesMatches) {
    ...
    console.log("Analyse du chevauchement:", {
      ...
      pourcentageVentesAvecStats:
        ((articlesCommuns.length / this.result.ventes.length) * 100).toFixed(
          2
        ) + "%",
    ...
    if (this.result.ventes.length > 0) {
      ...
      console.log(
        "Prix moyen de vente:",
        (totalCA / this.result.ventes.length).toFixed(2) + "€"
      );
      ...
      const ventesParMarque = this.result.ventes.reduce((acc: any, vente) => {
        ...
        const statArticle = this.result.ventes_stat.find(
          (stat) => stat.nom === vente.nom
        );
      ...
      Object.entries(ventesParMarque).forEach(
        ([marque, stats]: [string, any]) => {
          console.log(`\n${marque}:`, {
            nombreVentes: stats.nombreVentes,
            chiffreAffaires: stats.totalCA.toFixed(2) + "€",
            prixMoyenVente:
              (stats.totalCA / stats.nombreVentes).toFixed(2) + "€",
            vuesMoyennes:
              stats.vues > 0
                ? (stats.vues / stats.nombreVentes).toFixed(1)
                : "N/A",
            favorisMoyens:
              stats.favoris > 0
                ? (stats.favoris / stats.nombreVentes).toFixed(1)
                : "N/A",
            tauxConversion:
              stats.vues > 0
                ? ((stats.favoris / stats.vues) * 100).toFixed(2) + "%"
                : "N/A",
          });

          console.group("Détail des ventes");
          console.table(
            stats.ventes.map((v: any) => ({
              nom: v.nom,
      ...
      console.table(
        Object.entries(ventesParMarque)
          ...
          .sort(
            (a: any, b: any) =>
              parseFloat(b.chiffreAffaires) - parseFloat(a.chiffreAffaires)
          )
          .slice(0, 5)
      );
    }
    ...
    console.log(
      "Nombre total d'articles avec stats:",
      this.result.ventes_stat.length
    );

    // Regrouper par marque
    const statsParMarque = this.result.ventes_stat.reduce(
      (acc: any, article) => {
    ...
    Object.entries(statsParMarque).forEach(([marque, stats]: [string, any]) => {
      console.log(`\n🏷️ ${marque}:`, {
        ...
        tauxEngagement:
          ((stats.totalFavoris / stats.totalVues) * 100).toFixed(2) + "%",
      });
    ...
    console.table(
      Object.entries(statsParMarque)
        ...
        .sort(
          (a: any, b: any) =>
            parseFloat(b.tauxEngagement) - parseFloat(a.tauxEngagement)
        )
        .slice(0, 5)
    );
  ...
  private _extractFinances() {
    ...
    const transfertMatches = extractors.extractAll(
      patterns.transfert,
      this.text
    );
    ...
    console.log(
      ...
      this.result.finances.transferts.length
    );
    console.log(
      "Solde actuel:",
      this.result.finances.soldeActuel.toFixed(2) + " €"
