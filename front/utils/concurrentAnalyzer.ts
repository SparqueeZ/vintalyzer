...
class ConcurrentAnalyzer {
  ...
  async analyze(text: string): Promise<AnalyzeResults> {
    ...
    try {
      ...
      // Analyser les ventes
      const analyseVentes = this._analyseVentes(
        commentaires,
        boutique.evaluations,
        articles
      );
      console.log("💰 Analyse des ventes:", analyseVentes);

      // Calculer le nombre de pays avec des ventes significatives
      const paysAvecVentes = Object.entries(
        analyseVentes.ventesParPays || {}
      ).filter(([_, ventes]) => ventes > 0).length;
      ...
      const marques = new Set(
        articles
          .map((a) => a.marque)
          .filter((m) => m && m !== "Marque non spécifiée")
      );
  ...
  private _extractBoutiqueInfo(text: string): BoutiqueInfo {
    ...
    const info: BoutiqueInfo = {
      ...
      abonnes:
        parseInt((text.match(this.patterns.boutique.abonnes) || [])[1]) || 0,
      note: parseFloat((text.match(this.patterns.boutique.note) || [])[1]) || 0,
      articles:
        parseInt((text.match(this.patterns.boutique.articles) || [])[1]) || 0,
      evaluations:
        parseInt((text.match(this.patterns.boutique.evaluations) || [])[1]) ||
        0,
      localisation: {
        pays: "",
        ville: "",
  ...
  private _extractArticles(text: string): Article[] {
    ...
    while ((match = pattern.exec(text)) !== null) {
      const article: Article = {
        prix: this.rules.validation.prix(
          match[this.patterns.article.structure.prix]
        ),
  ...
  private _extractCommentaires(text: string): Commentaire[] {
    ...
    while ((match = pattern.exec(text)) !== null) {
      const commentaire: Commentaire = {
        ...
        langue: this.rules.validation.langue(
          match[this.patterns.commentaire.structure.contenu].trim()
        ),
      };
  ...
  private _analyseVentes(
    commentaires: Commentaire[],
    evaluations: number,
    articles: Article[]
  ): AnalyseVentes {
    // Analyser les ventes
    const analyseVentes = this.rules.analyse.analyseVentes(
      commentaires,
      evaluations
    );
    ...
    for (const [pays, nombre] of Object.entries(analyseVentes.ventesParPays)) {
      if (nombre > 0) {
        console.log(
          `- ${pays}: ${nombre} ventes (${analyseVentes.pourcentagesParPays[
            pays
          ].toFixed(1)}%)`
        );
      }
    ...
    console.log(
      `CA mensuel moyen estimé: ${(chiffreAffaires / 12).toFixed(2)}€`
    );
    ...
    const commentairesTriees = commentairesAvecDates.sort(
