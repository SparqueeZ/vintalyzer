    ...
    <div class="flex flex-col h-96">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center space-x-2">
          ...
          <h3 class="text-lg font-semibold text-white/90">Analyse des Ventes</h3>
        </div>
...
const filteredVentes = computed(() => {
  ...
  return store.analyzedData.ventes
    .filter(vente => {
      ...
      return (!startDate.value || date >= new Date(startDate.value)) && 
...
const totalRevenue = computed(() => {
  return filteredVentes.value.reduce((sum, vente) => sum + (vente.prix || 0), 0)
})
...
const periodDisplay = computed(() => {
  ...
  const last = new Date(filteredVentes.value[filteredVentes.value.length - 1].date)
...
watch([() => store.analyzedData, startDate, endDate], () => {
  if (hasVentes.value) {
...
onMounted(() => {
  if (store.analyzedData?.ventes?.length > 0) {
    ...
    const ventes = [...store.analyzedData.ventes].sort((a, b) => 
