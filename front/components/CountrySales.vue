  ...
  >
    <div class="flex flex-col h-full">
      ...
      <div class="flex items-center justify-center text-xs text-white/50 mt-2">
        <span
          class="inline-flex items-center px-2 py-1 bg-violet-500/20 text-violet-400 rounded-full"
        >
...
const internationalPercentage = computed(() => {
  if (!store.analyzedData?.boutique?.stats?.ventesParPays)
    return "Pas de ventes";

  const ventesParPays = store.analyzedData.boutique.stats.ventesParPays;
  const totalVentes = Object.values(ventesParPays).reduce(
    (acc, count) => acc + count,
    0
  );
