<template>
  <div v-if="hasStats" class="w-[640px] min-w-[640px] max-w-[640px] bg-[#1a1b23] rounded-lg p-4 transform transition-all duration-300">
    <!-- En-tête -->
    ...
    <div class="space-y-2">
      ...
      >
        <div v-for="brand in topBrands" :key="brand.name"
          class="px-2 py-2 rounded hover:bg-white/5 transition-colors duration-200">
          <div class="flex items-center">
            <!-- Nom marque -->
            <div class="flex-1">
              <span class="text-sm text-white/90 font-medium">{{ brand.name }}</span>
            </div>
